...
C***********************************************************************
      ...
      OPEN(UNIT=8,FILE='frogin_initial_sgr',STATUS='OLD')
      READ(8,*)nbods
      READ(8,*)nsteps,nout
      READ(8,*)dt
      CLOSE(8)

C Read the cartesian coordinage of Sgr postion.
      OPEN(UNIT=8,FILE='frog_sgr_initial.dat',STATUS='OLD')
      DO 10 i=1,nbods
...
C                -------
            CALL out
C               ----
            IF(i+nout.GT.nsteps) CALL out_final
C          --- Added by Joo H.
            CALL stepvel(dt2)
C                -------
         ELSE
...
C     output final position of particles   --- added by Joo H.
C***********************************************************************
      INCLUDE 'frog.h'
      REAL*8 e

C print out the initial position (at -2.95Gyr)
      OPEN(UNIT=991,FILE='sgr_initial',STATUS='UNKNOWN')
      DO i=1,nbods
         e=(vx(i)*vx(i)+vy(i)*vy(i)+vz(i)*vz(i))/2.+pot(i)
         e=(e-e0(i))/e0(i)
         WRITE(991,99)t,x(i),y(i),z(i),vx(i),vy(i),vz(i),e
      ENDDO
 99   FORMAT(8(1pe12.4))
C      CLOSE(999)
      RETURN
