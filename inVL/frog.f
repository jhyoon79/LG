...
C acceleration by subhalos
     ...
     &                       +(y(j)-y_subhalo(i))**2.
     &                       +(z(j)-z_subhalo(i))**2.)
c         if (r_subhalo_sat.gt.10.*rs_subhalo(i)) goto 111
c         if (r_subhalo_sat.lt.10.*rs_subhalo(i)) print *, i
c     &                         ,r_subhalo_sat/rs_subhalo(i)
...
C Initial energy --- added by J.H. Yoon
 ...
 5       CONTINUE
      ...
      sstring(5:5)=nstring(1+out_snap_dt/10000:1+out_snap_dt/10000)
c      sstring(5:5)=nstring(istring:istring)
