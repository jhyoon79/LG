...
c      OPEN(UNIT=8,FILE='frog_Pal5_xy.dat',STATUS='OLD')
      OPEN(UNIT=8,FILE='frog_Pal5.dat',STATUS='OLD')
      DO 10 i=1,nbods
...
c      OPEN(UNIT=9,FILE='frog_subhalo.dat',STATUS='OLD')
      OPEN(UNIT=9,FILE='orbit_subhalo.dat',STATUS='OLD')
      DO 20 i=1,nsubhalos
...
c            IF(MOD(out_snap_dt,nout_snap).EQ.0) THEN
            out_snap_dt = dt*i
            IF(MOD(i,nout_snap).EQ.0.and.t.gt.1370.and.t.lt.1450.) THEN
               CALL out_snap
...
C Initial energy --- added by J.H. Yoon
 ...
 5       CONTINUE
      ...
      sstring(1:4)='snap'
      istring=1+MOD(out_snap_dt*10,100000)/10000
c      sstring(5:5)=nstring(1+out_snap_dt/1000:1+out_snap_dt/1000)
      sstring(5:5)=nstring(istring:istring)
      istring=1+MOD(out_snap_dt*10,10000)/1000
c      sstring(6:6)=nstring(1+out_snap_dt*10/1000:1+out_snap_dt*10/1000)
      sstring(6:6)=nstring(istring:istring)
      istring=1+MOD(out_snap_dt*10,1000)/100
      sstring(7:7)=nstring(istring:istring)
      istring=1+MOD(out_snap_dt*10,100)/10
      sstring(8:8)=nstring(istring:istring)
      istring= 1+MOD(out_snap_dt*10,10)/1 + 0.1
      sstring(9:9)=nstring(istring:istring)

c      print *,sstring(1:9),' ',istring,' ',1+MOD(out_snap_dt*10,10)/1
      fname='snapshot_finedt/'//sstring(1:9)
...
C Initial energy --- added by J.H. Yoon
 ...
 5       CONTINUE
      ...
      sstring(1:7)='subhalo'
c      sstring(8:8)=nstring(1+out_snap_dt*10/100000:
c     +                     1+out_snap_dt*10/100000)
      istring=1+MOD(out_snap_dt*10,100000)/10000
c      sstring(8:8)=nstring(1+out_snap_dt/1000:1+out_snap_dt/1000)
      sstring(8:8)=nstring(istring:istring)
      istring=1+MOD(out_snap_dt*10,10000)/1000
c      sstring(9:9)=nstring(1+out_snap_dt/100:1+out_snap_dt/100)
      sstring(9:9)=nstring(istring:istring)
      istring=1+MOD(out_snap_dt*10,1000)/100
      ...
      fname='snapshot_subhalo_finedt/'//sstring(1:12)
      OPEN(UNIT=10,FILE=fname,STATUS='UNKNOWN')
