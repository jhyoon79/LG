...
t_present = where(abs(t_part1) eq t_now)
...
x_present = x_part1[t_present]  &  Vx_present = Vx_part1[t_present]
y_present = y_part1[t_present]  &  Vy_present = Vy_part1[t_present]
z_present = z_part1[t_present]  &  Vz_present = Vz_part1[t_present]
r_present = r_part1[t_present];  &  Vr_present = Vr_part1[t_present]
...
chr_rdtbl, 'Pal5_sigma.dat', 0, arr
...
device, file=dir_out+'Pal5_plot.ps', /color,/landscape
...
!p.multi=[0,1,2]
multiplot
plot, t_part1/1000., V_part1, ytitle=textoidl('V_{3D} [km/s]')
oplot, [t_part1[t_present]/1000.], [V_part1[t_present]], psym=7, color=255

multiplot
plot, t_part1/1000., r3D_part1,xtitle='t [Gyr]', ytitle='r [kpc]'
oplot, [t_part1[t_present]/1000.], [r3D_part1[t_present]], psym=7, color=255
multiplot, /reset
erase

!p.multi=[0,1,2]
multiplot
plot, t_part1/1000.,V_part1,xr=[0.1,0.2],ytitle=textoidl('V_{3D} [km/s]')
oplot, t_part1/1000.,V_part1,psym=2
multiplot
plot, t_part1/1000.,r3D_part1,xr=[0.1,0.2],xtitle='t [Gyr]', ytitle='r [kpc]'
oplot, t_part1/1000.,r3D_part1,psym=2
multiplot, /reset
erase
...
dr1[0] = 0
dr2[N_elements(r3D_part1)-1] = 0
apo = dr1*dr2
sub = where(apo lt 0)
m_orbit_r = mean(r3D_part1[sub])
apogee = where(r3D_part1[sub] gt m_orbit_r,complement=perigee)
...
sub_orbit = where(t_part1 le t_now,count)
!p.multi=[0,2,2]
plot, x_part1[sub_orbit], y_part1[sub_orbit], psym=0, xr=[xmin,xmax], yr=[ymin,ymax], xtitle='x [kpc]', ytitle='y [kpc]', title=Info1, /isotropic
oplot, [x_present], [y_present], psym=7, color=255
...
plot, y_part1[sub_orbit], z_part1[sub_orbit], psym=0, xr=[ymin,ymax], yr=[zmin,zmax], xtitle='y [kpc]', ytitle='z [kpc]', title=Info_posi, /isotropic
oplot, [y_present], [z_present], psym=7, color=255
...
plot, x_part1[sub_orbit], z_part1[sub_orbit], psym=0, xr=[xmin,xmax], yr=[zmin,zmax], xtitle='x [kpc]', ytitle='z [kpc]', title=Info_vel, /isotropic
oplot, [x_present], [z_present], psym=7, color=255
...
plot, r_part1[sub_orbit], z_part1[sub_orbit], psym=0, xr=[rmin,rmax], yr=[zmin,zmax], xtitle='r [kpc]', ytitle='z [kpc]', /isotropic
oplot, [r_present], [z_present], psym=7, color=255
xyouts, r_init, z_init, 't=0', charsize=1, color=255
...
!p.multi=[0,2,2]
sub = where(t_part1 gt t_now-500 and t_part1 lt t_now+500)
plot, x_part1[sub], y_part1[sub], psym=0, xr=[xmin,xmax], yr=[ymin,ymax], xtitle='x [kpc]', ytitle='y [kpc]', title=Info1, /isotropic
oplot, [x_present], [y_present], psym=7, color=255
xyouts, x_init, y_init, 't=0', charsize=1, color=255
xyouts, x_1Gyr, y_1Gyr, t_mark_1Gyr, charsize=1, color=255
xyouts, x_final, y_final, t_mark_final, charsize=1, color=255

plot, y_part1[sub], z_part1[sub], psym=0, xr=[ymax,ymin], yr=[zmin,zmax], xtitle='y [kpc]', ytitle='z [kpc]', title=Info_posi, /isotropic
oplot, [y_present], [z_present], psym=7, color=255
xyouts, y_init, z_init, 't=0', charsize=1, color=255
xyouts, y_1Gyr, z_1Gyr, t_mark_1Gyr, charsize=1, color=255
xyouts, y_final, z_final, t_mark_final, charsize=1, color=255

plot, x_part1[sub], z_part1[sub], psym=0, xr=[xmin,xmax], yr=[zmin,zmax], xtitle='x [kpc]', ytitle='z [kpc]', title=Info_vel, /isotropic
oplot, [x_present], [z_present], psym=7, color=255
xyouts, x_init, z_init, 't=0', charsize=1, color=255
xyouts, x_1Gyr, z_1Gyr, t_mark_1Gyr, charsize=1, color=255
xyouts, x_final, z_final, t_mark_final, charsize=1, color=255

plot, r_part1[sub], z_part1[sub], psym=0, xr=[rmin,rmax], yr=[zmin,zmax], xtitle='r [kpc]', ytitle='z [kpc]', /isotropic
oplot, [r_present], [z_present], psym=7, color=255
xyouts, r_init, z_init, 't=0', charsize=1, color=255
xyouts, r_1Gyr, z_1Gyr, t_mark_1Gyr, charsize=1, color=255
xyouts, r_final, z_final, t_mark_final, charsize=1, color=255
...
name = string(t_now,f='(i5.5)')
print, '---',name
chr_rdtbl, dir_out+'snapshot/snap'+name,0, arr
;chr_rdtbl, dir_out+'snapshot/snap12758',0, arr
arr = double(arr)
t = arr[0,*]
x = arr[1,*]
y = arr[2,*]
...
plot, x, y, psym=3, xtitle='x [kpc]', ytitle='y [kpc]', title='Pal 5', /isotropic
oplot, [x[0]], [y[0]], psym=1, color=255
plot, y, z, psym=3, xtitle='y [kpc]', ytitle='z [kpc]', /isotropic
oplot, [y[0]], [z[0]], psym=1, color=255
plot, x, z, psym=3,xtitle='x [kpc]', ytitle='z [kpc]', /isotropic
oplot, [x[0]], [z[0]], psym=1, color=255
...
sub_t = where(t_part1 gt t_now-400 and t_part1 lt t_now+400)
...
!p.multi=0
;plot,alpha,delta,xtitle=textoidl('\alpha [deg]'),ytitle=textoidl('\delta [deg]'),title=sigma,psym=8,symsize=0.2

plot,[0],[0],/nodata,xr=[-15,15],yr=[-6,6],xtitle=textoidl('\alpha [deg]'),ytitle=textoidl('\delta [deg]'),title=sigma
oplot,alpha-alpha[0],delta-delta[0],psym=8,symsize=0.2

plot,alpha,delta,psym=8,symsize=0.2

;plot,[0],[0],/nodata,xr=[247,218],yr=[-10,10],xtitle=textoidl('\alpha [deg]'),ytitle=textoidl('\delta [deg]'),title=sigma
plot,[0],[0],/nodata,xr=[247,224],yr=[-3,10],xtitle=textoidl('\alpha [deg]'),ytitle=textoidl('\delta [deg]'),title=sigma
...
for i=0,N_sub-1 do begin
  ...
  for k=0,N_in-1 do begin
    d2ortho_line = abs(1./fit[1]*alpha_in+1.*delta_in-alpha_in[k]/fit[1]-delta_in[k])/sqrt((1./fit[1])^2.+1.)
    del_y = d2line[k]-d2line[(sort(d2ortho_line))[1]]
    sum_del_y2 += del_y^2. 
    d2ortho_linev = abs(1./fit[1]*alpha_in+1.*Vr_in-alpha_in[k]/fit[1]-Vr_in[k])/sqrt((1./fit[1])^2.+1.)
    del_v = d2linev[k]-d2linev[(sort(d2ortho_linev))[1]]
    sum_del_v2 += del_v^2. 
  endfor
...
pts_contour,lcosb,b,20,40,gv,x,y
if (N_subhalos eq 0) then begin 
...
plot, lcosb, b, psym=3, xr=[max(lcosb),min(lcosb)], yr=[min(b),max(b)], xtitle='l cos b [deg]', ytitle='b [deg]', title=sigma
oplot, [lcosb_Pal5], [b_Pal5], psym=1, color=255,symsize=1.8,thick=8
...
Rperi = sqrt(x_peri^2.+y_peri^2.+z_peri^2.)
...
;  xyouts, 0,15, textoidl('\sigma=')+strtrim(string(sig,f='(f14.8)'),2),alignment=0.5
  ...
  scale_dJ = dJ / (r_tide/Rperi * J)
print,'ah'
print,epsilon
print,moment(r_tide/Rperi*J)
...
skip2:
  ...
  vline,0,linestyle=2
  hline,0,linestyle=2
  plot,q,scale_dJ,psym=3,xr=[-5,5],yr=[-4.5,4.5],xtitle='q',ytitle=textoidl('\DeltaJ / sJ')
  vline,0,linestyle=2
  hline,0,linestyle=2
  ;oplot,q[bottom_sub],scale_dJ[bottom_sub],psym=8,symsize=0.2,color=255
  
plothist,q,bin=0.1,xtitle='q'
...
;readf,6,R_peri
;close,6
;readcol,'R_peri',R_peri,R_apo
;print,R_peri
;tmp = min(abs(r-R_peri[0]),min_sub)
;tmp = min(abs(r-R_apo[0]),min_sub2)
;print, 'M(R<Rperi)=',r[min_sub],mr[min_sub]
;print, 'M(R<Rapo)=',r[min_sub2],mr[min_sub2]
print, 'M(R<25kpc)=',mr[where(r eq 25)]
...
;=== subhalo orbit ===
spawn, 'ls '+dir_out+'snapshot_subhalo > temp2'
chr_rdtbl, 'temp2', 0, fname
fname = dir_out+'snapshot_subhalo/'+reform(fname)
spawn, 'rm -f temp2'
...
spawn, 'ls '+dir_out+'snapshot > temp2'
chr_rdtbl, 'temp2', 0, fname
fname = dir_out+'snapshot/'+reform(fname)
spawn, 'rm -f temp2'
N_pts = N_elements(fname)-11
