...
C***********************************************************************
      INCLUDE 'frog.h'
      INTEGER i,j
      REAL*8 r,aror,c,phi0,mr,aror_km_s2
...
C********
      SUBROUTINE integ_NFW
      INCLUDE 'frog.h'
      REAL*8 rho,rhoc,dr,Nbin,dm,i
      PARAMETER (Nbin=1000.)
c      rhoc = Mhalo/(3.*(rs/rvir)**3.)
      rhoc = Mhalo/(4.*3.14159265*rs**3.)
      dr = 1./Nbin
      massN = 0.
      DO i=dr,p,dr
         dm = rhoc*1./(i*(1.+i)**2.)*4.*3.14159265*(i*rs)**2.*dr
         massN = massN+dm
      ENDDO
      RETURN
      END
