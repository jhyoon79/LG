...
dir_data = ['/scratch/jhyoon/Research/LG/Nbody_new/sgr/ten8/',$
           '/scratch/jhyoon/Research/LG/Nbody_new/sgr/ten6/',$
           '/scratch/jhyoon/Research/LG/Nbody_new/sgr/ten4/']
dir_data = dir_data[select]
...
;spawn,'ls '+dir_data2+'SNAP* > tmp5'
;chr_rdtbl,'tmp5',0,arr
;snap_fname2 = reform(arr[0,*])
;spawn,'rm -f tmp5'
...
set_plot,'ps'
@plot_setting
device,file=dir_out+'nbody_sgr.ps',/color
!p.multi=[0,1,2]
multiplot
...
!p.multi=[0,2,2]
plot,orbit_x,orbit_y,xr=[-70,70],yr=[-70,70],/isotropic
plot,orbit_y,orbit_z,xr=[-70,70],yr=[-70,70],/isotropic
plot,orbit_x,orbit_z,xr=[-70,70],yr=[-70,70],/isotropic
plot,sqrt(orbit_x^2.+orbit_y^2.),orbit_z,xr=[0,75],yr=[-70,70],/isotropic
erase

!p.multi=0
ocolor = color_code(orbit_t)
plot,[0],[0],/nodata,xr=[-70,70],yr=[-70,70],xtitle='x',ytitle='z',/isotropic
for i=0,N_orbit-1 do oplot,[orbit_x[i]],[orbit_z[i]],psym=8,symsize=0.3,color=ocolor[i]
nn = findgen(5) + 1
t_snap = [0,N_orbit*nn/max(nn)-1]
for i=0,N_elements(t_snap)-1 do begin
  tt = string(orbit_t[t_snap[i]],f='(f5.3)')+'Gyr'
  xyouts,orbit_x[t_snap[i]],orbit_z[t_snap[i]],tt,charsize=0.6,color=ocolor[t_snap[i]]
endfor

device,/close

N_file = N_elements(snap_fname);27
;N_file2 = N_elements(snap_fname2)
...
device,file=dir_out+'nbody_sgr_movie.ps',/color,ysize=20,yoffset=0.5
for i=0,N_file-1 do begin
  openr,1,snap_fname[i]
  readf,1,Npart,time
  close,1
  time = round(time*tu*1000.)/1000.*1000
  if time le 3400 then begin
...
;;=========
;if i le N_file2-1 then begin
;  chr_rdtbl,snap_fname2[i],1,arr,/silent
;  arr = double(arr)
;  if (Npart eq 3000) then begin
;    sub_sample = round(randomu(11,3000)*(N_elements(arr[0,*])-1))
;    arr = arr[*,sub_sample]
;  endif
;  m_nbody = arr[0,*] * mu
;  x_nbody2 = arr[1,*] * ru; / r_tide
;  y_nbody2 = arr[2,*] * ru; / r_tide
;  z_nbody2 = arr[3,*] * ru; / r_tide
;  x2 = x_nbody2
;  y2 = costheta*y_nbody2 + sintheta*z_nbody2
;  z2 = -sintheta*y_nbody2 + costheta*z_nbody2
;  ox_nbody2[i,*] = cosphi*x2 - sinphi*z2
;  oy_nbody2[i,*] = y2
;  oz_nbody2[i,*] = sinphi*x2 + cosphi*z2
;endif  
;=======================


  xmin = -70;min([min(x_mine),min(x_nbody)])
  xmax = 70;max([max(x_mine),max(x_nbody)])
  ymin = -70;min([min(y_mine),min(y_nbody)])
  ymax = 70;max([max(y_mine),max(y_nbody)])
  zmin = -70;min([min(z_mine),min(z_nbody)])
  zmax = 70;max([max(z_mine),max(z_nbody)])
...
;=== find particle escaped at the 4th latest perigee ===
t_apo_cri1 = t_apo[5];5
t_apo_cri2 = t_apo[6];6
  if (time/1000. lt t_apo_cri1) then pre_tunbound_nbody_ex_4peri = tunbound_nbody
  if (time/1000. gt t_apo_cri1 and time/1000. lt t_apo_cri2) then $
    sub_4peri = where(tunbound_nbody ne 0 and pre_tunbound_nbody_ex_4peri eq 0)
  if (time/1000. gt t_apo_cri1) then begin
    x_nbody_ex_4peri = x_nbody[sub_4peri]
    y_nbody_ex_4peri = y_nbody[sub_4peri]
    z_nbody_ex_4peri = z_nbody[sub_4peri]
    vx_nbody_ex_4peri = vx_nbody[sub_4peri]
    vy_nbody_ex_4peri = vy_nbody[sub_4peri]
    vz_nbody_ex_4peri = vz_nbody[sub_4peri]
    pot_int_nbody_ex_4peri = pot_int_nbody[sub_4peri]
    pot_ext_nbody_ex_4peri = pot_ext_nbody[sub_4peri]
    tunbound_nbody_ex_4peri = tunbound_nbody[sub_4peri]
    Etot_nbody_ex_4peri = (vx_nbody_ex_4peri^2.+vy_nbody_ex_4peri^2.+vz_nbody_ex_4peri^2.)/2.+pot_ext_nbody_ex_4peri+pot_int_nbody_ex_4peri
    J_nbody_ex_4peri = sqrt((y_nbody_ex_4peri*vz_nbody_ex_4peri-z_nbody_ex_4peri*vy_nbody_ex_4peri)^2. + $
                    (z_nbody_ex_4peri*vx_nbody_ex_4peri-x_nbody_ex_4peri*vz_nbody_ex_4peri)^2. + $
                    (x_nbody_ex_4peri*vy_nbody_ex_4peri-y_nbody_ex_4peri*vx_nbody_ex_4peri)^2.)
    dE_nbody_ex_4peri = Etot_nbody_ex_4peri-m_Etot_nbody
    dJ_nbody_ex_4peri = J_nbody_ex_4peri-m_J_nbody
    q_nbody_ex_4peri = dE_nbody_ex_4peri/epsilon_nbody
    sJ_nbody_ex_4peri = s*J_nbody_ex_4peri
;    sJ_nbody_ex_4peri = r_tide/r_peri_mine*J_nbody_ex_4peri
    dJ_sJ_nbody_ex_4peri = dJ_nbody_ex_4peri/sJ_nbody_ex_4peri

; particle coord. in the orbital plane ===
    x2 = x_nbody_ex_4peri
    y2 = costheta*y_nbody_ex_4peri + sintheta*z_nbody_ex_4peri
    z2 = -sintheta*y_nbody_ex_4peri + costheta*z_nbody_ex_4peri
    Nex = N_elements(x2)
    ox_nbody_ex_4peri[i,0:Nex-1] = cosphi*x2 - sinphi*z2
    oy_nbody_ex_4peri[i,0:Nex-1] = y2
    oz_nbody_ex_4peri[i,0:Nex-1] = sinphi*x2 + cosphi*z2
  endif
...
;  sJ_nbody = r_tide/r_peri_mine*J_nbody
  ...
  legend,[string(time/1000,f='(f6.3)')+'Gyr'],box=0

  plot,x_mine,y_mine,psym=3,xr=[xmin,xmax],yr=[ymin,ymax],xtitle='x [kpc]',ytitle='y = [kpc]',/isotropic


  plot,x_nbody,z_nbody,psym=3,xr=[xmin,xmax],yr=[zmin,zmax],xtitle='y [kpc]',ytitle='z = [kpc]',/isotropic
  plot,x_mine,z_mine,psym=3,xr=[xmin,xmax],yr=[zmin,zmax],xtitle='y [kpc]',ytitle='z = [kpc]',/isotropic
  ...
  pts_contour,q_nbody[escaped],dJ_sJ_nbody[escaped],25,25,gv,xx,yy
...
device,file=dir_out+'nbody_sgr_snap.ps',/color,ysize=16
!p.multi=[0,5,6]
!p.charsize=1
;for i=0,N_file-1 do begin
for i=0,N_file-1,1 do begin
  ...
  endif else if mu eq 1e6 then begin
  ...
  endif else if mu eq 1e8 then begin
  ...
  plot,[0],[0],/nodata,xr=[-23.2,23.2],yr=[-21.6,20.9],title=tname,/isotropic
...
;  oplot,new_xo,yfit,color=255
  legend,[string(snap_time[i]/1000,f='(f6.3)')+'Gyr'],box=0,charsize=0.6,position=[0,-0.5]
endfor
multiplot,/reset
device,/close
erase

device,file=dir_out+'nbody_sgr_current_snap.ps',/color,ysize=25
!p.multi=[0,1,6]
!p.charsize=1
;multiplot
plot,orbit_t,orbit_r,ytitle='r [kpc]'
...
;  multiplot
  ...
  plot,[0],[0],/nodata,xr=[-70,70],yr=[-70,70],/isotropic
...
;  oplot,new_xo,new_yo,color=220
;  oplot,new_xo,yfit,color=255
  if j eq 0 then legend,[string(snap_time[i]/1000,f='(f6.3)')+'Gyr',tname],box=0
endfor
multiplot,/reset
device,/close
erase

device,file=dir_out+'nbody_sgr_sky.ps',/color,ysize=15
!p.multi=0
for i=0,N_file-1 do begin
