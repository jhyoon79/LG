...
cC NFW potential
c         r_subhalo=SQRT(x_subhalo(i)**2.+y_subhalo(i)**2.
c     &             +z_subhalo(i)**2.)
c         p_subhalo=r_subhalo/rs
c         mr_subhalo=Mhalo*(dlog(p_subhalo+1.)-p_subhalo/(p_subhalo+1.))
cC truncation of NFW halo
c         if (r.gt.rvir) mr_subhalo = Mhalo*(dlog(c+1.)-c/(c+1.))
c         aror_subhalo=-(G*mr_subhalo/r_subhalo**2.)
c         pot_subhalo(i)=-(phi0/p_subhalo)*dlog(p_subhalo+1.)
cC
c         aror_subhalo_km_s2 = aror_subhalo/kpc2km
c         ax_subhalo(i)=aror_subhalo_km_s2*x_subhalo(i)/r_subhalo
c         ay_subhalo(i)=aror_subhalo_km_s2*y_subhalo(i)/r_subhalo
c         az_subhalo(i)=aror_subhalo_km_s2*z_subhalo(i)/r_subhalo
c 20   CONTINUE
...
c max delE
         Vtail = sqrt(Vx(j)**2.+Vy(j)**2.+Vz(j)**2.)
         Vrel = sqrt((Vx_subhalo(i)-Vx(j))**2.
     &          +(Vy_subhalo(i)-Vy(j))**2.+(Vz_subhalo(i)-Vz(j))**2.)
         delE = G*mr_subhalo_sat/(r_subhalo_sat*Vrel)*Vtail
         if(delE.ge.max_delE(i)) max_delE(i)=delE
      ENDDO
      RETURN
      END
...
C Read in test particle positions and velocities
      OPEN(UNIT=8,FILE='frog_rotate2xy_5.5Gyr.new',STATUS='OLD')
      DO 10 i=1,nbods
...
C***********************************************************************
      ...
      OPEN(UNIT=997,FILE=out_dir//'part_peri_xy',STATUS='UNKNOWN')
      DO i=1,nbods
...
C Initial energy --- added by J.H. Yoon
 ...
 5       CONTINUE
      ...
      fname='snapshot_subhalo_xy/'//sstring(1:12)
      OPEN(UNIT=10,FILE=out_dir//fname,STATUS='UNKNOWN')
...
C
            fname=sstring(1:7)
            OPEN(UNIT=10+i,FILE=out_dir//fname//'_xy',STATUS='UNKNOWN')
